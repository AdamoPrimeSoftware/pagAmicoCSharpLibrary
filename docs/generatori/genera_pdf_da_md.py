...
documenti generati a mano (genera_documentazione.py e genera_guida_prove.py).
    ...
    python generatori\genera_pdf_da_md.py                 rigenera i PDF gia' esistenti
    python generatori\genera_pdf_da_md.py quadro-01-*.md  solo quelli indicati (basta il nome)

I sorgenti stanno in docs\md\<tipo>\, i PDF vanno in docs\pdf\<stesso tipo>\.
...
def pdf_per(md_path):
    """Il PDF va in pdf/ nella stessa sottocartella del sorgente: md/guide/x.md -> pdf/guide/x.pdf."""
    rel = os.path.relpath(os.path.abspath(md_path), MD)
    out = os.path.join(PDF, os.path.splitext(rel)[0] + ".pdf")
...
def main(argv):
    if argv:
        ...
        for a in argv:
            if os.path.isabs(a):
                trovati = glob.glob(a)
            elif "/" in a or os.sep in a:
                trovati = glob.glob(os.path.join(DOCS, a))
            else:
                # basta il nome del file: si cerca in tutte le sottocartelle di md/
                trovati = glob.glob(os.path.join(MD, "*", a))
            if not trovati:
                print("nessun file per: %s" % a)
            sorgenti += trovati
    else:
        # senza argomenti: rigenera i PDF che esistono gia', lasciando i .md che stanno solo in md/
        sorgenti = [md for md in sorted(glob.glob(os.path.join(MD, "*", "*.md")))
                    if os.path.exists(pdf_per(md))]
